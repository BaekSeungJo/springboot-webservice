...
import com.devbaek.book.springboot.config.auth.dto.OAuthAttributes;
import com.devbaek.book.springboot.config.auth.dto.SessionUser;
import com.devbaek.book.springboot.domain.user.User;
import com.devbaek.book.springboot.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // 로그인 서비스를 진행 중인 서비스 구분 코드 ( ex : google )
        String registrationId = userRequest.getClientRegistration().getRegistrationId();
        // OAuth2 로그인 진행 시 키가 되는 필드값 ( PK와 같은 역활 수행, ex : google - sub )
